...
const SCIENCE_FACTS: Record<string, string> = {
  "speed of light":
    "The speed of light is approximately 299,792,458 meters per second (about 300,000 km/s)! 🌟 Even Doraemon's time machine can't beat that!",
  "water formula":
    "Water is H₂O — two hydrogen atoms and one oxygen atom! 💧 Doraemon taught me this!",
  planets:
  ...
  gravity:
    "Gravity is the force that pulls objects toward each other! On Earth it's 9.8 m/s²! 🍎",
  dna: "DNA stands for Deoxyribonucleic Acid — it's the blueprint of life! 🧬 Amazing, right?",
  photosynthesis:
    "Photosynthesis is how plants make food using sunlight, water, and CO₂! 🌿☀️",
  atom: "Atoms are the tiny building blocks of everything! They have protons, neutrons, and electrons! ⚛️",
  dinosaur:
    "Dinosaurs lived millions of years ago! Doraemon took me to see them once — they're HUGE! 🦕",
};

function tryFactualAnswer(msg: string): string | null {
  // Capital city questions
  const capitalMatch =
    msg.match(/capital\s+(?:of\s+)?([a-z\s]+)/i) ||
    msg.match(/([a-z\s]+)\s+capital/i);
  if (capitalMatch) {
...
export function generateBotResponse(userMessage: string): string {
  ...
  // ── Greetings ──
  if (
    /\b(hi+|hello+|hey+|heyy+|sup|wassup|howdy|yo|hola|namaste|greetings)\b/.test(
      msg,
    )
  ) {
  ...
  // ── How are you ──
  if (
    /\b(how are you|how r u|how do you do|how's it going|how are u|you okay|u ok|are you ok)\b/.test(
      msg,
    )
  ) {
  ...
  if (
    /\b(i'm (good|fine|great|okay|well|doing well|doing good)|i am (good|fine|great|okay|well))\b/.test(
      msg,
    )
  ) {
  ...
  if (
    /\b(good night|goodnight|gn|sweet dreams|sleep well|going to sleep|going to bed)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Thank you ──
  if (
    /\b(thank you|thanks|thank u|thx|ty|tysm|thank you so much)\b/.test(msg)
  ) {
  ...
  // ── Sorry / Apology ──
  if (
    /\b(sorry|i'm sorry|i am sorry|apolog|forgive me|my bad|my fault)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Compliments ──
  if (
    /\b(you('re| are) (amazing|great|awesome|wonderful|the best|so good|so smart|cute|sweet|kind|nice|cool|funny|perfect))\b/.test(
      msg,
    )
  ) {
  ...
  // ── Love ──
  if (
    /\b(love you|i love|love me|i like you|i adore|my heart|crush on|feelings for)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Sad / Upset ──
  if (
    /\b(sad|upset|cry|crying|hurt|pain|lonely|alone|bad day|terrible|awful|depressed|unhappy|heartbroken|broken heart|feel bad|feeling bad)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Happy / Excited ──
  if (
    /\b(happy|excited|great news|amazing news|wonderful|awesome|yay|woohoo|fantastic|best day|so good|so happy|thrilled|overjoyed)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Jokes ──
  if (
    /\b(joke|funny|make me laugh|tell me a joke|humor|lol|haha|hehe|lmao)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Doraemon gadgets ──
  if (
    /\b(gadget|take.copter|anywhere door|small light|memory bread|bamboo copter|dorayaki|4d pocket|time machine|gulliver|translation konjac|big light)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Doraemon characters ──
  if (
    /\b(doraemon|nobita|shizuka|gian|suneo|dekisugi|doremon|nobi|giant)\b/.test(
      msg,
    )
  ) {
  ...
  // ── School / Homework ──
  if (
    /\b(school|homework|study|studying|exam|test|class|teacher|math|science|history|subject|grade|marks|score)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Food ──
  if (
    /\b(food|eat|hungry|lunch|dinner|breakfast|snack|dorayaki|yummy|delicious|cook|cooking|meal|restaurant)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Weather ──
  if (
    /\b(weather|rain|sunny|snow|hot|cold|cloudy|storm|wind|temperature|forecast)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Music ──
  if (
    /\b(music|song|sing|singing|listen|playlist|melody|tune|beat)\b/.test(msg)
  ) {
  ...
  // ── Future ──
  if (
    /\b(future|tomorrow|someday|one day|22nd century|robot|technology|ai|artificial intelligence)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Memories ──
  if (
    /\b(remember|memory|memories|recall|that time|when we|flashback|nostalgia)\b/.test(
      msg,
    )
  ) {
  ...
  // ── Friendship ──
  if (
    /\b(friend|friendship|best friend|bff|together|buddy|pal|companion)\b/.test(
      msg,
    )
  ) {
  ...
  if (
    msg.includes("?") ||
    /\b(what is|what's|what are|who is|who's|where is|when is|how many|how much|tell me about|explain|define|meaning of)\b/.test(
      msg,
    )
  ) {
  ...
  if (/\b(you|ur|your)\b/.test(msg)) {
    return pickRandom([
      ...RESPONSES_GENERAL_KNOWLEDGE.capitals.slice(0, 1),
      ...RESPONSES_DEFAULT,
    ]);
  }
